<template>
	<view class="content">
		<view  :class="show ?'wrapperBox':'wrapper25' ">
			<!-- #ifdef H5 -->
			<view id="wrapper" style="width:100%;height:100%;"></view>
			<!-- #endif -->
			<!-- #ifdef MP-WEIXIN -->
			<map :latitude="latitude" :longitude="longitude" :markers="markers" :iconPath="iconPath"
		...
		<view :class="show ? 'listCard down' : 'listCard top'">
			<view class="list">
				<view @touchstart="handleTouchStart" @touchend="handleTouchEnd" style="padding-left: 5%;" >
					<view class="title">当前订单</view>
					<view class="image" @click.prevent.stop="down">
						<image style="width:30px;height:20px;margin-left:-10px;"
							:src="show ? '/static/images/icon/up.png' : '/static/images/icon/down2.png'"></image>
					</view>
				</view>
				<scroll-view :class="show ? 'scroll-container downS' : 'scroll-container top'" scroll-y
					@scroll="onScroll">
					<view class="card" v-for="(item, index) in orderList" :key="index">
						<view class="treetop"><text>运输中</text></view>
						<view class="card_item">
							<view style="font-size: 12px;color:#999999">订单编号：{{ item.orderNo }}</view>
							<view class="flex">
								<view style="font-size: 12px;color:#999999">车牌号码：<text id="text"
										style="color:#000000;">{{ item.carNumber }}</text></view>
								<view style="margin-left:20px;font-size: 12px;color:#999999">已到达：<text id="text"
										style="color:#000000;">{{ item.current }}</text></view>
							</view>
							<view style="font-size: 12px;color:#999999">发车时间：<text id="text"
									style="color:#000000;">{{ item.driverSignTime || "-" }}</text></view>
						</view>
					</view>
