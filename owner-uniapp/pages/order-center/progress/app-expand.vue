<template>
	<view>
		...
		<map class="map-view" id="map1" ref="map1" :show-location="false" :latitude="latitude" :longitude="longitude"
			...
			<cover-view @click="expandDrawer"  v-if="false" ref="Item"  class="tipNvue" :style="'background-color: red;'+styleCss()">11111111111111</cover-view>
		 </map>
		<!-- #endif -->
		...
		<ww-bottom-drawerapp ref="drag" :proportionShow='proportionvc' :dragHandleHeight="handleHeight" :isExpand="mExpand" 
			...
			<slot>
				...
				<scroll-view :scroll-top="scrollTop" :scroll-y="mExpand" :style="'width: 100%;height :'+  (menuHeight + 'px')  "
					...
					<view @click.stop="clickAli">测试支付宝是否透传</view>
					...
					<uni-grid :column="4" :highlight="false" :showBorder="false" @change="onChange">
						<uni-grid-item v-for="(item, index) in 80" :index="index" :key="index">
