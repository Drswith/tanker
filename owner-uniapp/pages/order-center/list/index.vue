...
const tabList = [
  { status: OrderStatus.Created, operations: ['取消订单', '立即支付'] }, // 0 - 已创建待支付
  { status: OrderStatus.Paid, operations: ['取消订单', '立即支付'] }, // 1 - 已支付待接单
  { status: OrderStatus.Signed, operations: [] }, // 3 - 已签署司机前往发车地待验车
  { status: OrderStatus.Accepted, operations: ['取消订单', '修改订单', '立即签署'] }, // 2 - 已接单待签署（业主和平台）
  // { status: OrderStatus.Verified, operations: ['寄回GPS'] }, // 4 - 验车通过待施封
  // { status: OrderStatus.Unverified, operations: [] }, // 5 - 验车不通过
  // {  status: OrderStatus.Sealed , operations: [] }, // 6 - 完成施封待安装GPS
  // {status: OrderStatus.GpsInstalled , operations: [] }, // 7 - 完成GPS安装待司机签署
  { status: OrderStatus.DriverSigned, operations: ['订单进度', '验收授权', '确认收货'] }, // 8 - 司机已签署（运输中）
  // {  status: OrderStatus.DeliveryConfirmed , operations: [] }, // 9 - 司机确认送达待核验
  { status: OrderStatus.WaitingGpsReturn, operations: ['寄回GPS'] }, // 13 - 确认收货后待邮寄GPS
  { status: OrderStatus.OwnerVerified, operations: ['立即评价'] }, // 10 - 业主核验确认收货后待评价
  // {  status: OrderStatus.OwnerRejected , operations: [] }, // 11 - 业主核验不通过
  // { status: OrderStatus.Evaluated , operations: [] }, // 12 - 已评价（用于前端查询）
  // {  status: OrderStatus.GpsShipped , operations: [] }, // 14 - 已邮寄
  { status: OrderStatus.GpsReceived, operations: [] }, // 15 - 后台确认收到GPS订单结束
  // {  status: OrderStatus.RefundSubmitted , operations: [] }, // 16 - 已提交资料待退款
  { status: OrderStatus.RefundCompleted, operations: ['删除订单'] }, // 17 - 已退款已取消
]
...
export default {
  ...
  data() {
    return {
      ...
      tabList: Object.freeze(tabList.map(item => ({
        ...item,
